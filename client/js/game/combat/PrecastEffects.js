...
function healPowerHands(gamePiece, obj3d) {
    ...
    let effectCb = function(efct) {
        ...
        let startSize = 0.1;
        let endSize = size*2.50
        let time = CombatFxUtils.setupLifecycle(efct, 0.4+Math.random()*0.3, 0.07, 0.5);

        efct.activateSpatialTransition(ThreeAPI.tempVec3, efct.quat, obj3d.position, tempObj.quaternion, startSize, endSize, time, CombatFxUtils.endOnLanded, 0.1, 0.1)
    }
    ...
    let shockwaveCb = function(efct) {
        ...
        efct.activateSpatialTransition(ThreeAPI.tempVec3, tempObj.quaternion, obj3d.position, tempObj.quaternion, startSize, endSize, time, CombatFxUtils.endOnLanded, MATH.randomBetween(-0.1, 0.1), MATH.randomBetween(-0.1, 0.1))
    }
